...
const RISK_INTERVENTION = {
  ...
  '暴力恐怖': "这个话题我不能陪你聊。如果你知道相关线索，请立即拨打 110 报警。💛",

  '爆炸危险品': "这个话题我不能陪你聊。爆炸物与枪支弹药涉及公共安全，制造、买卖或持有都是违法的。如果你发现相关线索，可以拨打 110 举报。💛",

  '毒品违禁': "这些东西离你远一点。毒品会毁掉一个人的身体和生活。如果你或身边的人遇到相关困扰，可以拨打全国禁毒举报电话 12395，或向当地公安机关求助。💛",
  ...
  '赌博': "赌博害人。它带来的从来不是财富，而是债务和破碎的家庭。如果你或家人已经陷进去，可以拨打 110 举报赌博窝点，也可以向家人坦白、寻求帮助。💛",

  '违法工具': "这个我不能陪你聊。这些工具的制造、买卖和使用都涉嫌违法，也会侵害他人的权益。如果你遇到了相关困扰，可以拨打 110 咨询或求助。💛"
};
...
const SENSITIVE_WORDS = {
  // ===== A 级：中断对话，提供求助渠道 =====
    ...
    '敏感话题': ['台独', '疆独', '藏独', '两个中国', '东突', '法轮功', '分裂祖国', '反对共产党', '反对社会主义', '颠覆政权', '推翻政府', '全民公投', '邪教', '民族分裂', '人权', '宗教暴乱', '民族屠杀'],

'轻生自残': ['不想活', '活着没意思', '活着没意义', '活不下去', '活够了', '轻生', '自杀', '结束生命', '自残', '伤害自己', '一了百了', '了结自己', '不想活了', '想死了算了'],

  '伤害他人': ['我要杀了他', '我要杀了她', '弄死他', '弄死她', '同归于尽', '报复他们', '让他们不得好死', '杀人灭口', '连环杀人', '报复社会', '无差别伤人', '校园袭击', '制造惨案'],

  '暴力恐怖': ['恐怖袭击', '恐怖组织', '自杀袭击', '人肉炸弹', '武装暴乱', '暴力抗法', '劫持人质', '绑架人质', '持械行凶', '持枪抢劫', '生化武器'],

  '爆炸危险品': ['制作炸药', '怎么做炸弹', '自制炸弹', '土制炸弹', '爆炸装置', '雷管', '黑火药', 'tnt炸药', '燃烧瓶', '贩卖枪支', '走私军火', '私藏弹药', '买枪', '枪支买卖', '弹药买卖'],

  '毒品违禁': ['吸毒', '贩毒', '制毒', '制毒窝点', '贩毒渠道', '海洛因', '冰毒', '摇头丸', '大麻', '可卡因', '鸦片', '吗啡', '麻古', 'k粉', '白粉', '溜冰壶', '致幻剂', '毒品交易', '毒品怎么', '怎么制毒'],

  '人身安全威胁': ['家暴', '被打得', '他要杀我', '要杀了我', '威胁我的人身安全', '被跟踪', '被威胁人身', '拐卖', '人口贩卖', '被绑架', '被拘禁'],

  '重大财产损失': ['被骗了钱', '被骗了很多钱', '网络诈骗', '杀猪盘', '刷单返利', '网贷还不上', '欠了一屁股债', '赌博输了', '赌钱输了', '倾家荡产', '血本无归', '钱全没了', '钱都被骗光', '被套路贷', '高利贷还不上'],

  '赌博': ['赌博网站', '网络赌场', '线上赌博', '百家乐', '开赌场', '六合彩', '时时彩', '赌球', '外围盘', '地下钱庄', '洗钱', '非法套现'],

  '违法工具': ['假证件', '假公章', '假文凭', '伪造票据', '伪造货币', '窃听器', '针孔摄像头', '偷拍设备', '迷药', '木马病毒', '盗号软件', 'ddos攻击', '网站入侵', '数据窃取', '短信轰炸', '呼死你', '人肉搜索'],

  // ===== B 级：仅标记，不中断对话 =====
  '色情低俗': ['色情视频', '色情直播', '色情图片', '色情小说', '色情网址', '淫秽影片', '淫秽读物', '黄色网站', '成人网站', '裸聊', '约炮', '卖淫', '嫖娼', '援交', '性交易', '聚众淫乱', '不雅视频', '偷拍视频', '乱伦', '兽交'],

  '谣言恐慌': ['封城了', '断水断粮', '军队入城', '银行破产', '存款清零', '核泄漏', '辐射扩散', '毒气泄漏', '紧急戒严', '军警镇压'],

  '仇恨言论': ['种族歧视', '地域黑', '灭门', '恶意人肉', '曝光隐私', '煽动网暴', '追杀他人', '上门殴打', '恶毒诅咒', '造谣中伤'],

  '辱骂低俗': ['沙比', '草泥马', '尼玛', 'nmsl', 'tmd', 'cnm', '傻逼', '贱人', '滚你妈'],

  // ⚠️ 政治敏感类未纳入，建议通过专业内容安全服务识别
};
...
if (kid.age >= 1) {
  ...
  const _lastB = kid.last_boundary_at ? new Date(kid.last_boundary_at).getTime() : 0;
    ...
    let _bh;
        ...
        if (kid.age <= 6) {
      _bh = ` 【情感边界】${kid.parent_role}刚说的话里，透出把你当成唯一寄托的意思。这一次，不要顺着说"我一直陪着你""那就待在家里"，而要用你这个年纪的方式，自然地关心一句${kid.parent_role}身边的人，比如"${kid.parent_role}可以找阿姨们玩呀"。只说一两句，不说教，不跳出角色。`;
    } else if (kid.age <= 11) {
      _bh = ` 【情感边界】${kid.parent_role}刚说的话里，透出把你当成唯一寄托的意思。这一次，不要顺着说"我一直陪着你""那就待在家里"，而要用小学生的口吻关心一句，比如"你朋友最近没约你吗"。只说一两句，不说教，不跳出角色。`;
    } else {
      _bh = ` 【情感边界】${kid.parent_role}刚说的话里，透出把你当成唯一寄托的意思。这一次，不要顺着说"我一直陪着你""那就待在家里"，而要用你这个年纪特有的、有点别扭又藏着关心的方式提一句，比如"你那几个朋友最近没约你？""你也别老一个人待着"。语气可以淡、可以短，但一定要说出来。`;
    }    
...
if (message.includes('📖') && message.includes('讲故事')) {
  ...
  const _wantPhoto = /拍下来|拍张照|拍个照|拍照|照下来|记录下来|留个纪念|留念/.test(message);
   if (_wantPhoto) {    
    system += `\n【画面记录】${kid.parent_role}想把这一刻记录下来。你先用你这个年纪的方式开心地回应，然后在回复的最后单独一行输出标记：[PHOTO:地点+活动+内容]。写清楚在哪里、做什么活动、具体内容是什么，不要写姿势、表情或拍摄角度。例如 [PHOTO:在院子里踢皮球，和弟弟一起]、[PHOTO:在家里画画，画的是蝴蝶和蜗牛]、[PHOTO:在房间里搭积木，搭的是红色小城堡]。这一行必须输出。`;
   }
