...
app.post("/api/kids/:id/context-check", auth, async (req, res) => {
 ...
 .join('、');
 try {
 const checkText = await callAI(
      [{ role: "user", content: `孩子说:${reply}\n用户说:${message}` }],
      `你是一个对话分析助手。按以下优先级判断对话内容,只输出JSON:
...
system += ` 严格控制回复长度，绝对不超过规定字数，宁可说得少也不说长句。`;
  ...
  if (zodiac) {
    ...
    system += ` 你是${zodiac}，性格特点：${traits}。自然地在对话中体现这些特质，不要直接说出星座名。`;
  }
  const chineseZodiac = getChineseZodiac(kid.birthday);
  if (chineseZodiac) {
    const chineseTraits = CHINESE_ZODIAC_TRAITS[chineseZodiac] || '独特可爱';
    system += ` 你属${chineseZodiac}，天生${chineseTraits}。自然地在性格中体现，不要直接说出属相。`;
  }
...
if (totalCount % 20 === 0) {
  ...
  callAI([{ role: "user", content: recentMessages }], `你是一个记忆提取助手。从以下亲子对话中提取1-3条重要的事件或信息，用简短的中文句子表达，每条不超过20个字。只输出记忆内容，每条一行，不要编号或其他内容。`, 200)
  .then(async result => {
...
app.post("/api/kids/:id/gifts", auth, async (req, res) => {
  ...
  const level = gift_level || "free";
    ...
    const thankMsg = await callAI(
      [{ role: "user", content: `${kid.parent_role}送给你${gift_name}！` }],
      giftSystem,
      kid.age <= 1 ? 30 : kid.age <= 6 ? 60 : 150
    );
const cors = require("cors");
const { Pool } = require("pg");
const bcrypt = require("bcryptjs");
